...
# --- CRITICAL: THE MISSING HOME ROUTE ---
@app.route('/')
def index():
...
def cluster():
    try:
        ...
        return jsonify({'plot_data': {'windows': [{'cluster': int(l), 'data': w['data'].tolist(), 'median': w['median'].tolist()} for l, w in zip(labels, windows)]}, 'elbow_data': elbow_data})
    except Exception as e:
...
# --- SEARCH ROUTES (Optional but recommended for sidebar) ---
