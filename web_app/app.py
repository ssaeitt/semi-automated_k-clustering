...
def process_data(file_data, sheet_name='Sheet1'):
    """Extracts raw data and prepares normalized vectors for clustering."""
    try:
...
def create_windows(data, window_size):
    ...
    for i in range(0, n - window_size + 1, window_size):
        ...
        if window_data.shape[0] < 2: break
        median = np.median(window_data, axis=0)
        slope, _ = np.polyfit(window_data[:, 0], window_data[:, 1], 1) if np.ptp(window_data[:, 0]) != 0 else (0.0, 0)
        windows.append({'data': window_data, 'median': median, 'slope': slope, 'index': len(windows)})
    return windows
...
def custom_distance(w1, w2, D_max, T_max, le, lp, b, gamma_block=1.0):
    idx_diff = abs(w1['index'] - w2['index'])
    # Euclidean
    e_dist = np.linalg.norm(w1['data'].flatten() - w2['data'].flatten())
    ...
    # Block Bonus
    bonus = -gamma_block if w1.get('inverted_block') and w2.get('inverted_block') else 0.0
    
    return max(le * norm_e + lp * norm_p + b * norm_t + bonus, 0)

@app.route('/get_preview', methods=['POST'])
def get_preview():
    """Returns coordinates based on the Data Preview selection."""
...
def cluster():
    try:
        ...
        le, lp, b = params.get('lambda_e', 1.0), params.get('lambda_p', 1.0), params.get('beta', 0.5)
        w_size = int(params.get('window_size', 5))
        
        # Prepare windowed data
        data_stack = np.column_stack((current_data['x_norm'], current_data['y_norm']))
        windows = create_windows(data_stack, w_size)
        ...
        for i in range(len(windows)):
            for j in range(i+1, len(windows)):
                d = np.linalg.norm(windows[i]['data'].flatten() - windows[j]['data'].flatten())
                if d > D_max: D_max = d
        ...
        for i in range(n_w):
            for j in range(i, n_w):
                v = custom_distance(windows[i], windows[j], D_max, T_max, le, lp, b, params.get('gamma_block', 1.0))
                dist_mat[i, j] = dist_mat[j, i] = v

        elbow_data = None
        if method == 'kmeans':
            feats = [[le*w['median'][0], le*w['median'][1], lp*w['slope'], b*w['index']] for w in windows]
            ...
            if backbone == 'kmedoids':
                labels = KMedoids(n_clusters=k_opt, metric='precomputed', random_state=42).fit_predict(dist_mat)
            else:
                labels = KMeans(n_clusters=k_opt, random_state=42).fit_predict(X_s)

        # Prepare response (Re-ordering clusters by time sequence)
        clusters = np.unique(labels)
        # Use window medians instead of raw data to avoid dimension mismatch
        window_x_coords = np.array([w['median'][0] for w in windows])
        ...
        return jsonify({'plot_data': {'windows': [{'cluster': int(l)} for l in labels]}, 'elbow_data': elbow_data})
    except Exception as e:
